...
public class EgovBBSManageController {
	...
	//return "backoffice/sub_menu/publication/publication_list2";
	
		if (!boardVO.getBbsId().equals("Publication")) {
		    return "backoffice/sub_menu/news/news_list";
		}else {
    ...
    public String selectBoardArticle(@ModelAttribute("searchVO") BoardVO boardVO, ModelMap model) throws Exception {
		...
		if (!boardVO.getBbsId().equals("Publication")) {
		    return "backoffice/sub_menu/news/newsInqire";
		}else {
	...
	if (!boardVO.getBbsId().equals("Publication")) {
	    return "backoffice/sub_menu/news/NewsRegist";
	}else {
	...
	if (!boardVO.getBbsId().equals("Publication")) {
	    return "backoffice/sub_menu/news/newsUpdt";
	}else {
    ...
    public static Map<String, Object> convertToMap(Object vo) throws IllegalAccessException {
    ...
    @RequestMapping("/backoffice/publication/selectUniconList.do")
    public String selectUniconPublication(@ModelAttribute("searchVO") BoardVO boardVO, ModelMap model) throws Exception {
	LoginVO user = (LoginVO)EgovUserDetailsHelper.getAuthenticatedUser();
	boardVO.setBbsId("Publication");
	//boardVO.setBbsId(boardVO.getBbsId());
	boardVO.setBbsNm(boardVO.getBbsNm());
	...
	return "backoffice/sub_menu/publication/publication_list";
    }
    
    
    
    @RequestMapping("/backoffice/publication/addUniconArticle.do")
    public String addUniconPublication(@ModelAttribute("searchVO") BoardVO boardVO, ModelMap model) throws Exception {
	// 사용자권한 처리
	if(!EgovUserDetailsHelper.isAuthenticated()) {
		model.addAttribute("message", egovMessageSource.getMessage("fail.common.login"));
    	return "cmm/uat/uia/EgovLoginUsr";
	...
	return "backoffice/sub_menu/publication/publication_edit";
    }
